...
-- EMPLOYEE 테이블의 전체 사원들의 사버느 이름, 급여만을 조회
...
SELECT EMP_NAME AS 이름, SALARY AS 월급, HIRE_DATE AS 입사일
FROM EMPLOYEE
WHERE SALARY >= 3000000;

-- 2. EMPLOYEE 테이블에서 연봉이 5000만원 이상인 직원의 직원명, 급여, 연봉, 입사일 조회
SELECT EMP_NAME AS 이름, SALARY AS 월급, SALARY*12 AS 연봉, HIRE_DATE AS 입사일
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 급여가 350만원 이상 600만원 이하를 받는 직원의 아닌 사번, 직원명, 부서 코드, 급여 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 이름 중에 '하'가 포함된 사원명, 주민번호, 부서 코드 조회
...
-- EMPLOYEE 테이블에서 이메일 중 _앞 글자가 3자리인 이메일 주소를 가진 사원의 사번, 사원명, 이메일 조회
...
----------------- 실습 문제 -------------------
-- 1. EMPLOYEE 테이블에서 이름 끝이 '연'으로 끝나는 사원의 사원명, 입사일
...
/*
    <IS NULL / IS NOT NULL>
        [표현법]
            ...
            - 비교대상컬럼 IS NULL : 컬럼 값이 NULL인 경우 TRUE를 리턴한다.
            - 비교대상컬럼 IS NOT NULL : 컬럼 값이 NULL이 아닌 경우 TRUE를 리턴한다.
*/
...
-- EMPLOYEE 테이블에서 부서 배치를 받진 않았지만 보너스는 받는 사원의 사원명, 부서코드, 보너스 조회
...
/*
    <ORDER BY>
        [표현법]
            ...
            FROM 테이블명
            WHERE 조건식
            ORDER BY 정렬 시키고자 하는 컬럼명|별칭|컬럼 순번 [ASC|DESC] [NULLS FIRST|NULLS LAST];
...
SELECT *
FROM EMPLOYEE
ORDER BY BONUS NULLS FIRST; -- 오름차순 정렬은 기본적으로 NULL LAST

-- EMPLOYEE 테이블에서 BONUS로 내림차순 정렬 (단, 보너스 값이 일치할 경우 그때는 SALARY를 가지고 오름차순으로 정렬)
SELECT *
FROM EMPLOYEE
--ORDER BY BONUS DESC NULLS LAST; -- 내림차순 정렬은 기본적으로 NULL FIRST
