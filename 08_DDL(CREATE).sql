/*
    <DDL(Data Defination Languague)>
        데이터 정의 언어로 오라클에서 제공하는 객체를 만들고(CREATE), 변경하고(ALTER), 삭제하는(DROP) 등
        실제 데이터 값이 아닌 데이터의 구조 자체르르 정의하는 언어로 DB 관리자, 설계자가 주로 사용한다.
        
        * 오라클에서의 객체 : 테이블, 뷰, 시퀀스, 인덱스, 트리거, 프로시져, 함수, 동의어, 사용자 등등
...
-- 위에서 테이블에 추가한 데이터를 실제 데이터베이스에 반연한다. (메모리 버퍼에 임시 저장된 데이터를 실제 데이블에 반영)
COMMIT;
...
CREATE TABLE MEMBER (
    MEMBER_ID VARCHAR2(20) NOT NULL,
    MEMBER_PWD VARCHAR2(20) NOT NULL,
    MEMBER_NAME VARCHAR2(20) CONSTRAINT MEMBER_MEMBER_NM NOT NULL,
    MEMBER_DATE DATE DEFAULT SYSDATE,
...
INSERT INTO MEMBER VALUES('2', 'USER2', '1234', '문인수', '강', 125, DEFAULT);
INSERT INTO MEMBER VALUES('3', 'USER3', '1234', '홍길동', '여', -30, DEFAULT);
...
/*
    <PRIMARY KEY(기본 키) 제약조건>
        ...
        PRIMARY KRY 제약조건을 설정하게 되면 자동으로 해당 컬럼에 NOT NULL + UNIQUE 제약조건이 설정된다.
...
CREATE TABLE MEMBER (
    ...
    AGE NUMBER CHECK(AGE > 0),
    MEMBER_DATE DATE DEFAULT SYSDATE,
    CONSTRAINT MEMBER_MEMBER_NO_PK PRIMARY KEY(MEMBER_NO, MEMBER_ID)    -- 컬름을 묶어서 하나의 기본 키를 설정 -> 복합키하고 한다.
);
...
-- 기본 키로 설정된 커럼은 NULL 값이 있으면 에러가 발생한다.
...
/*
    <FOREIGN KEY(외래 키) 제약조건>
        ...
        [표현법]
            1) 컬럼 레벨
                컬럼명 자료형(크기) [CONSTRAINT 제약조건명] REFERENCES 참조테이블명 [(기본 키)] [삭제룰]
            2) 테이블 레벨
                [CONSTRAINT 제약조건명] FOREIGN KEY(컬럼명) REFERENCES 참조테이블명 [(기본 키)] [삭제룰]
                
        [삭제룰]
            부모 테이블의 데이터가 삭제됐을 때의 옵션은 지정해 놓을 수 있다.
            1) ON DELETE RESTRICT : 자식 테이블의 참조 키가 부모 테이블의 키 값을 참조하는 경우 주어진 상위 행을 삭제할 수 없다. (기본적으로 적용 된느 옵션)
...
-- 50이라는 값이 MEMBER_GRADE 테이블 GRADE_CODE 컬럼에서 지공하는 값이 아니므로 외래 키 제약 조건에 위배되어 오류 발생
INSERT INTO MEMBER VALUES('2', 'USER2', '1234', '문인수', '남', 30, 50, DEFAULT);
INSERT INTO MEMBER VALUES('3', 'USER3', '1234', '홍길동', '남', 20, NULL, DEFAULT); -- GRADE_ID 컬럼에 NULL 사용 가능
...
INSERT INTO MEMBER VALUES('1', 'USER1', '1234', '춘향이', '여', 20, 10, DEFAULT);
INSERT INTO MEMBER VALUES('3', 'USER3', '1234', '홍길동', '남', 20, NULL, DEFAULT);

-- 문제없이 행이 삭제되는 것을 확인할 수 있다.
-- 단, 자식 테이블을 조회에 보면 사제된 행을 참조하고 있던 컬럼의 값이 NULL로 변경된 것을 확인할 수 있다.
DELETE FROM MEMBER_GRADE WHERE GRADE_CODE = 10;
...
-- EMPLOYEE 테이블을 복사한 새로운 테이블 생성 (컬럼, 데이터 타입, 제약 조건을 복사)
CREATE TABLE EMPLOYEE_COPY2
AS SELECT * 
   FROM EMPLOYEE
   WHERE 1 = 0; -- 모든 행에 대해서 매번 FALSE이기 때문에 구조만 복사되고 데이터 값은 복사되지 않는다.
...
CREATE TABLE TB_PUBLISHER (
    ...
    PUB_NAME VARCHAR2(30) CONSTRAINT TB_PUBLISHER_PUB_NAME_NN NOT NULL,
    PHONE VARCHAR2(12)
);
-- 컬럼에 주석 달기
...
CREATE TABLE TB_MEMBER (
    ...
    MEMBER_ID VARCHAR2(30) CONSTRAINT TB_MEMBER_MEMBER_ID_UQ UNIQUE,
    ...
    GENDER CHAR(3) CONSTRAINT TB_MEMBER_GENDER_C CHECK(GENDER IN ('M', 'F')),
    ADDRESS VARCHAR2(30),
    PHONE VARCHAR2(30),
    STATUS CHAR(3) DEFAULT 'N' CONSTRAINT TB_MEMBER_STATUS_C CHECK(STATUS IN ('N', 'Y')),
...
JOIN TB_BOOK TB ON TB.BK_NO = TR.RENT_BOOK_NO
WHERE TB.BK_NO = 2;

-- 5. 회원번호가 1번인 회원이 대여한 도서들의 도서명, 출판사명, 대여일, 반납예정일을 조회하시오.
SELECT TB.BK_TITLE AS "도서명", TP.PUB_NAME AS "출판사명", TR.RENT_DATE AS "대여일", (TR.RENT_DATE + 7) AS "반납 예정일"
