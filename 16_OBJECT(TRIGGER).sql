/*
    <TRIGGER>
        테이블에 INSERT, UPDATE, DELETE 등 DML 구문에 의해서 변경될 경우
        자동으로 실행될 내용을 정의해놓은 객체이다.
        ...
        * 트리거의 종류
            ...
            2) SQL 문에 의해 영향을 받는 행에 따른 분류
                - 문장 트리거(STATEMENT TRIGGER) : 해당 QGL 문에 대해 한 번만 트리거를 실행한다.
                - 행 트리거 (ROW TRIGGER)       : 해당 SQL 문에 영향을 받는 행마다 트리거를 실행한다. (FOR EACH ROW 옵션)
...
-- 상품코드 중복되지 않게 새로운 번호를 발생하는 시퀀스 객체를 생성한다.
...
CREATE TABLE PRODETAIL(
    ...
    PCODE NUMBER,                           -- 상품코드(외래 키로 지정, PRODUCT 테이블을 탐조)
