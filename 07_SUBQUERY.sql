...
-- 1) 전 직원의 평균 급여보다 적게 받는 직원들의 이름, 직급 코드, 급여 조회
-- 전 직원 평균 급여
SELECT AVG(SALARY)
FROM EMPLOYEE;  -- 1행 1열 (3047662.60869565217391304347826086956522)

SELECT EMP_NAME, JOB_CODE, TO_CHAR(SALARY, 'L99,999,999')
FROM EMPLOYEE
WHERE SALARY < (
                SELECT AVG(SALARY) 
                FROM EMPLOYEE
...
-- 2) 최저 급여를 받는 직원의 사번, 이름, 직급 코드, 입사일 조회
...
-- 3) 노옹철 사원의 급여보다 더 많이 받는 사원의 사번, 사원명, 부서명, 직급 코드, 급여 조회(노옹철 급여 -> 3,700,000)
...
-- 4) 부서별 급여의 합이 가장 큰 부서의 부서 코드, 급여의 합 조회(같이) (D9 -> 합계 17,700,000)
...
FROM EMPLOYEE E
JOIN JOB J ON E.JOB_CODE = J.JOB_CODE
WHERE JOB_NAME = '대리' AND SALARY > ANY(2200000, 2500000, 3760000); -- SALARY > 220만 OR  SALARY > 250만 OR SALARY > 376만
...
-- 4) 과장 직급임에도 차장 직급의 급여보다 더 많이 받는 직원들의 사번, 이름, 직급, 급여 조회
SELECT SALARY
FROM EMPLOYEE E
JOIN JOB J ON E.JOB_CODE = J.JOB_CODE
...
FROM EMPLOYEE E
JOIN JOB J ON E.JOB_CODE = J.JOB_CODE
WHERE J.JOB_NAME = '과장' AND SALARY > ALL (2800000, 1550000, 2490000, 2480000); -- SALARY>280만 AND SALARY>155만 AND SALARY>249만 AND SALARY>248만
...
/*
    <RANK 함수>
        [표현법]
            ...
            RANK() OVER(정렬 기준) : 동일한 순위 이후의 등수를 동일한 인원수만큼 건너뛰고 순위를 계산한다. (ex.공동 1위가 2명이명 그 다음 순위는 3위)
