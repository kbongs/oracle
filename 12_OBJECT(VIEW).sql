...
-- 한국에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 근무 국가명 조회하시오.
SELECT *
FROM V_EMPLOYEE
...
DELETE FROM V_EMP
WHERE DEPT_TITLE = '총무부';   -- 서브 쿼리에 FROM절의 테이블에만 영향을 끼친다.
...
/*
    <VIEW 옵션>
        [표현법]
            ...
            - OR REPLACE : 기존에 동일한 뷰가 있을 경우 덮어쓰고, 존재하지 않으면 부를 새로 생성한다.
            ...
            - WITH READ ONLY OPTION : 뷰에 대해 조회만 가능(DML 수행 불가
*/
...
-- 200 사원의 급여르르 200만원으로 변경 (서브 쿼리의 조건에 부합하지 않기 때문에 변경이 불가능하다. )
UPDATE V_EMP03
SET SALARY = 2000000
WHERE EMP_ID = 200;

-- 200 사원의 급여르르 400만원으로 변경 (서브 쿼리의 조건에 부합하기 때문에 변경이 가능하다. )
UPDATE V_EMP03
SET SALARY = 4000000
WHERE EMP_ID = 200;
