/*
    <PL/SQL>
        오라클 자체에 내장되어 있는 절차적 언어로 SQL 문장 내에서 변수의 정의, 조건 처리(IF), 반복 처리(LOOP, FRO, WHILE) 등을 지원한다.
        (다수의 SQL 문을 한 번에 실행이 가능하다.)
        
        [PL/SQL의 구조]
            1) 선언부(DECLARE SECTION)         : DECLARE로 시작, 변수나 상수를 선언 및 초기화하는 부분이다.
            2) 실행부(EXCUTABLE SECTION)       : BEGIN로 시작, SQL 문, 제어문(조건, 반복문) 등의 로직을 기술하는 부분이다.
...
BEGIN
    ...
    INTO EID, ENAME, SAL
    FROM EMPLOYEE
    WHERE EMP_NAME = '&NAME';   -- &(엠퍼샌드) 기호는 대체 변수(값을 입력)을 입력하기 위한 창을 띄어주는 구문이다.
...
/*
            3) IF ~ ELSIF ~ ELSE 구문
                [표현법]
                    ...
                    END IF;
*/
-- 점수를 입력받아 SCORE 변수에 저장한 후 학점은 입결된 점수에 따라 GRADE 변수에 저장한다.
--  90점 이상은 'A'
--  80점 이상은 'B'
--  70점 이상은 'C'
--  60점 이상은 'D'
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('해당 사원의 급여 등급은 ' || SGRADE || '입니다');
...
DECLARE
    SAL EMPLOYEE.SALARY%TYPE;
    SGRADE SAL_GRADE.SAL_LEVEL%TYPE;
BEGIN
...
/*
    3) 예외처리부(EXCEPTION SECTION)
        ...
        * 오라클에서 미리 정의되어 있는 예외(시스템 예외)
            ...
            - TOO_MANY_ROWS     : 한 행이 리턴되야하는데 SELET 문에서 여러 개의 해을 반환할 때 발생한다.
