...
*/
-- 표현법 1번을 테스트할 테이블만들기(테이블 구조만 복사)
...
-- 표현법 2_번을 테스트할 테이블 만들기(테이블 구조만 복사)
...
/*
    <UPDATE>
        ...
        [표현법]
            ...
            -- UPDATE 시에 서브 쿼리를 사용해서 서브 쿼리를 수행항 결과값으로 컬럼의 값을 변경할 수 있다.(WHERE 절에도 서브 쿼리 사용가능)
*/
-- 테스트 진행할 테이블 생성
...
-- 선동일 사원의 급여를 7000000원으로 , 보너스를 0.2로 변경
UPDATE EMP_SALARY
SET SALARY = 7000000,
    BONUS = 0.2
WHERE EMP_NAME = '선동일';
...
-- EMP_SALARY 테이블에서 아시아 지역에 근무하는 직원의 보너스를 0.3변경
SELECT EMP_ID
FROM EMPLOYEE E
...
DELETE FROM DEPARTMENT
WHERE DEPT_ID = 'D3';   -- D3의 값을 참조하는 자식 테이블 데이ㅓ가 없기 때문에 삭제가 된다.
...
TRUNCATE TABLE EMP_SALARY;  -- ROLLBAKC이 불가능하다.
