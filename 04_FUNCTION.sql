/*
    <함수>
        컬럼값을 읽어서 계산한 결과를 반환한다.
            - 단일행 함수 : N 개의 값을 읽어서 N 개의 결과를 리턴한다. (매 행 함수 실행 결과를 반환)
...
/*
        2) INSTR
            - INSTR(컬럼|'문자값','문자값' [, PORSITION[, OCCURRENCE]])
...
SELECT EMAIL, INSTR(EMAIL, '@') AS "@위치", INSTR(EMAIL, 's', 1, 2) AS "S위치"
FROM EMPLOYEE;

/*
        3) LPAD / RPAD
            - LPAD/RPAD(컬럼|'문자값', 최종적으로 반환할 문자의 길리(바이트)[, 덧붙이고자 하는 문자])
            - 제시한 컬럼|'문자값'에 임의의 문자를 왼쪽 또는 오른족에 덧붙여 최종 N 길이 만큼의 문자열을 반환한다.
...
/*
    5) TRIM
        - TRIM([[LEADING|TRAILING|BOTH] 제거하고자하는 문자값 FROM] 컬럼|'문자값')
...
SELECT SUBSTR('SHOWMEHTEMONEY', 7) FROM DUAL;
SELECT SUBSTR('SHOWMEHTEMONEY', 5, 2) FROM DUAL;
SELECT SUBSTR('SHOWMEHTEMONEY', 1, 6) FROM DUAL;
SELECT SUBSTR('SHOWMEHTEMONEY', -8, 3) FROM DUAL;
...
SELECT CONCAT('가나다라', 'ABCD', '1234') FROM DUAL; -- 에러 발생(CONCAT은 두 개의 문자열만 전달)
...
/*
    9) REPLACE
        - REPLACE(컬럼|'문자값', 변경하고자 하는 문자, 변경하고자 하는 문자)
...
/*
    3) ROUND
        [표현법]
            ...
            - 반올림해주는 함수
            - 위치 : 기본0(.), 양수(소수점 기준으로 오른쪽)와 음수(소주점 기준으로 왼쪽)로 입력가능
*/
...
/*
        4) NEXT_DAY
            [표현법]
                ...
                - 특정 날짜에 구하려는 요일의 가장 가까운 날짜를 리턴한다.
                - 결과값은 DATE이다.
*/
...
SELECT SYSDATE, NEXT_DAY(SYSDATE, '금') FROM DUAL;   -- 에러 발생(현재 언어가 AMERICAN이기 때문에)
...
SELECT EMP_NAME, HIRE_DATE, LAST_DAY(HIRE_DATE) AS "입사월의 마지막 날짜"
FROM EMPLOYEE;
...
-- 일에 대한 포맷
SELECT TO_CHAR(SYSDATE, 'D'),   -- 1WNFMF 기준으로 며칠째
...
SELECT TO_NUMBER('10,000,000', '99,999,999') + TO_NUMBER('500,000', '99,999,999') FROM DUAL;
    ...
    <NULL 처리 함수>
        ...
        2) NVL2
            [표현법]
                ...
                - 컬럼 값이 NILL이 아니면 바꿀값 1, 컬럼값이 NULL 이면 바꿀값 2로 변경하여 반환한다.
        2) NULLIF
            [표현법]
                NULLIF(비교대상 1, 비교대상2)
...
-- EMPLOYEE 테이블에서 사원명, 부서 코드를 조회 (단, 부서 코드가 NILL이면 "부서없음" 출력)
SELECT EMP_NAME, NVL(DEPT_CODE, '부서없음')
FROM EMPLOYEE;

-- 보너스를 동결하고 싶을 때
SELECT EMP_NAME, NVL2(BONUS, 0.1, 0), (SALARY+(SALARY*NVL2(BONUS, 0.1, 0))) * 12 AS "연봉"
FROM EMPLOYEE;
...
/*
    <선택함수>
        ...
        1) DECODE
            [표현법]
                DECODE(컬럼|계산식, 조건값 1, 결과값1, 조건값 2, 결과값 2, ..., 결과값)
...
SELECT EMP_NAME,
       JOB_CODE,
       TO_CHAR(SALARY, '99,999,999') AS "월급",
       TO_CHAR(DECODE(JOB_CODE, 'J7', SALARY*0.1, 'J6', SALARY*0.15, 'J5', SALARY*0.2, SALARY*0.05), '99,999,999') AS "인상될 금액",
...
-- EMPLOYEE 테이블에서 남자 사원 총 급여의 합계
SELECT SUM(SALARY) AS " 남자 사원 총 급여"
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO,8,1) = 1;

-- EMPLOYEE 테이블에서 여자 사원 총 급여의 합계
SELECT SUM(SALARY)
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO,8,1) = 2;
