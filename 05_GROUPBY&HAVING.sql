...
SELECT JOB_CODE AS "직급 코드",
       TO_CHAR(FLOOR(AVG(NVL(SALARY,0))), 'L99,999,999') AS "급여 평균"
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO,8,1), 1,'남',2,'여') -- 컬럼, 계산식, 함수 호출 구문이 올 수 있다. (단, 컬럼 순번, 별칭은 사용할 수 없다.)
ORDER BY "성별";
...
SELECT DEPT_CODE, FLOOR(AVG(NVL(SALARY,0))) --SALARY 
FROM EMPLOYEE
...
-- CUBE(컬럼1, 컬럼2, ...) -> 컬럼 1을 가지고 중간집계를 내고, 컬럼 2를 가지고도 중간집계도 낸다. 즉, 전달되는 컬럼 모두 집계한다.
SELECT DEPT_CODE, JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
