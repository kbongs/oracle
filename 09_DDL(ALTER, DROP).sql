/*
    ...
    <테이블 수정>
        [표현법]
            ...
            * 수정할 내용
                1) 컬럼 추가/수정 삭제
                2) 제약 조건 추가/삭제 -> 수정은 불가능(삭제한 후 새로 추가해야 된다.)
                3) 테이블명/컬러명/제약조건명 변경
*/
-- 실습에 사용할 테이블 생성
...
-- 1-3) 컬럼 삭제(DROP COLIMN) : ALTER TABLE 테이블명 DROP COLUMN 컬럼명
--  데이터 값이 기록되어 있어도 같이 삭제된다. (단, 삭제된 컬럼 복구는 불가능)
--  테이블에는 최소 한 개의 컬럼은 존재해야한다.
...
ALTER TABLE DEPT_COPY DROP COLUMN LOACTION_ID; -- 최소 한 개의 컬럼은 있어야 한다.
...
--      FOREIGN KEY : ALTER TABLE 테이블명 ADD [CONSTRAINT 제약조건명] FOREIGN KEY(컬럼명) REFERENCES 테이블명 [(컬렴명)];
...
MODIFY DEPT_TITLE CONSTRAINT DEPT_COPY_TITLE_NN NOT NULL;
...
-- 3) 테이블명/컬럼명/제약조건명 변경
--  3-1) 컬럼명 변경     : ALTER TABLE 테이블명 RENAME COLUMN 기존커럶명 TO 변경할컬럼명
-- DEPT_COPY 테이블에 DEPT_TITLE -> DEPT_NAME으로 변경
ALTER TABLE DEPT_COPY RENAME COLUMN DEPT_TITLE TO DEPT_NAME;

SELECT * FROM DEPT_COPY;

--  3-2) 제약조건명 변경  : ALTER TABLE 테이블명 RENAME CONSTRAINT 기존제약조건명 TO 변경할제약조건명
-- SYS_C007157 -> DEPT_COPY_DEPT_NAME_NN
ALTER TABLE DEPT_COPY MODIFY DEPT_NAME NOT NULL; -- 제약조건 하나 추가
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C007157 TO DEPT_COPY_DEPT_NAME_NN;

--  3-3) 테이블명 변경
--      1) ALTER TABLE 테이블명 RENAME TO 변경할테이블명
--      2) RENAME 기존테이블명 TO 변경할테이블명
-- DEPT_COPY -> DEPT_TEST
ALTER TALBE DEPT_COPY RENAME TO DEPT_TEST;
